...
export default async function AdminDashboard() {
  ...
  const stats = [
    { label: 'Total Complaints', value: totalComplaints, color: '#ffffff', bg: 'rgba(255,255,255,0.12)', border: 'rgba(255,255,255,0.2)' },
    { label: 'Pending', value: pendingComplaints, color: '#fde68a', bg: 'rgba(245,193,60,0.15)', border: 'rgba(245,193,60,0.35)' },
    { label: 'In Progress', value: inProgressComplaints, color: '#93c5fd', bg: 'rgba(59,130,246,0.15)', border: 'rgba(59,130,246,0.35)' },
    { label: 'Resolved', value: resolvedComplaints, color: '#86efac', bg: 'rgba(34,197,94,0.15)', border: 'rgba(34,197,94,0.35)' },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-white/60 mt-1 text-sm">Welcome back — here's what's happening today.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map(({ label, value, color, bg, border }) => (
          ...
          >
            <p className="text-sm font-semibold uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.65)' }}>{label}</p>
      ...
      {/* ── Recent Complaints ── */}
      <div className="rounded-2xl shadow-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', backdropFilter: 'blur(16px)' }}>
        <div className="px-6 py-4 flex justify-between items-center" style={{ borderBottom: '1px solid rgba(255,255,255,0.10)', background: 'rgba(0,0,0,0.15)' }}>
        ...
        {recentComplaints.length === 0 ? (
          <p className="p-8 text-center" style={{ color: 'rgba(255,255,255,0.5)' }}>No complaints yet.</p>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(0,0,0,0.10)' }}>
                  {['Location', 'Type', 'Status', 'Date'].map(h => (
                    <th key={h} className="px-6 py-3 text-xs font-semibold uppercase tracking-wider" style={{ color: 'rgba(255,255,255,0.55)' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {recentComplaints.map(c => (
                  ...
                  >
                    <td className="px-6 py-4 text-sm font-semibold" style={{ color: '#ffffff' }}>
